...
static void handle_unix_client(int client_fd)
{
    ...
    if (msg_len!=sizeof(proc_request_t)) {
        (void)fprintf(stderr, "[unixds] Unexpected msg_len %u (expected %zu)\n",
...
int run_unix_domain_server(void)
{
    ...
    if (bind(server_fd, (struct sockaddr *)&addr, sizeof(addr))!=0) {
        (void)perror("[unixds] bind");
        (void)close(server_fd);
        return -1;
    }

    if (listen(server_fd, UNIX_BACKLOG)!=0) {
        (void)perror("[unixds] listen");
        (void)close(server_fd);
        return -1;
    }

    (void)fprintf(stdout, "[unixds] Unix domain socket server: %s\n", UNIX_SOCKET_PATH);
