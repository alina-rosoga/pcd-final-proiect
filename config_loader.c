...
int config_load(const char*path, server_config_t*out)
{
    ...
    out->log_level=1;
    (void)snprintf(out->output_dir, MAX_PATH_LEN, "%s", "./output");
    (void)snprintf(out->soap_endpoint, MAX_PATH_LEN, "%s", "http://localhost:8080/soap");
    ...
    if (config_lookup_string(&cfg, "server.output_dir", &odir)==CONFIG_TRUE) {
        (void)snprintf(out->output_dir, MAX_PATH_LEN, "%s", odir);
    }

    /* Server.soap_endpoint */
    const char*soap_ep;
    if (config_lookup_string(&cfg, "server.soap_endpoint", &soap_ep)==CONFIG_TRUE) {
        (void)snprintf(out->soap_endpoint, MAX_PATH_LEN, "%s", soap_ep);
    }
