...
/* implicit endpoint - can be overridden by env var PCD_SERVER         */
...
{
    /* mediu variabile */
    const char*env_server=getenv(ENV_SERVER_KEY); /* called before threads start, MT-safe here */
    ...
    /* parseaza argumente */
    int opt;
    while ((opt=getopt(argc, argv, "s:f:m:r:h"))!=-1) { /* called before threads start, MT-safe here */
        switch (opt) {
        case 's': {
            endpoint=optarg;
            break;
