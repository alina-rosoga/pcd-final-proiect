...
static ssize_t read_all(int sock_fd, void*buf, size_t len)
{
    ...
    while (rem > 0) {
        ...
        ptr +=num_bytes; rem -=(size_t)num_bytes;
    }
    return (ssize_t)len;
}
                    ...
                    proc_result_t*res)
{
    ...
    if (connect(fd, addr_info->ai_addr, addr_info->ai_addrlen)!=0) {
    ...
    (void)write_all(fd, &len_net, sizeof(len_net));
    (void)write_all(fd, req, sizeof(*req));
    ...
    (void)read_all(fd, res, sizeof(*res));
    (void)close(fd);
    return 0;
}
...
int main(int argc, char*argv[])
{
    ...
    const char*host=getenv(ENV_RAW_SERVER); /* single-threaded, safe */
    if (!host) { host="127.0.0.1"; }

    const char*port_env=getenv(ENV_RAW_PORT); /* single-threaded, safe */
    ...
    int opt;
    while ((opt=getopt(argc, argv, "h:p:f:m:r:?"))!=-1) { /* single-threaded, safe */
        switch (opt) {
        case 'h': {
            host=optarg;
            break;
        ...
        case 'r': {
            char *endptr;
            sr=(int)strtol(optarg, &endptr, STRTOL_BASE);
            break;
        }
        case '?': {
            break;
    ...
    if (raw_client_call(host, port, &req, &res)!=0) {
        (void)fprintf(stderr, "[inetsample2] Call failed\n");
        return 1;
    }
